...
export class Renderer {
  ...
  /**
   * The scene from the light: depth for the solid things (hardscape, body,
   * plants), then the fins' transmitted light. The tank itself is left out — the sand and
  ...
  private renderShadows(time: number): void {
    ...
    this.bodyGpu.draw();
    // The leaves' shadows sway with them.
  ...
  private drawSceneGeometry(time: number): void {
    ...
    // And only in the pass for what is out of the water, which all of it is.
    if (!this.mirrored && this.pane.x === 0 && this.pane.y === 0 && this.pane.z === 0) {
      gl.disable(gl.CULL_FACE);
